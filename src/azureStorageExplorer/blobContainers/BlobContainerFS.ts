...
export class BlobContainerFS implements vscode.FileSystemProvider {
    ...
    async writeFile(uri: vscode.Uri, content: Uint8Array, options: { create: boolean; overwrite: boolean; }): Promise<void> {
        await callWithTelemetryAndErrorHandling('blob.writeFile', async (context) => {
            ...
            let parentPath: string = path.posix.join(parsedUri.rootPath, parsedUri.parentDirPath);
            parentPath = parentPath.replace(/\/$/, "");
            if (this._virtualDirCreatedUri.has(parentPath)) {
                this._virtualDirCreatedUri.delete(parentPath);
            }
        });
    }
