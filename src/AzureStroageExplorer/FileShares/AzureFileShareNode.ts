...
import { AzureFileNode } from './AzureFileNode';
import { AzureDirectoryNode } from './AzureDirectoryNode';
import * as azureStorage from "azure-storage";
import * as path from 'path';

export class AzureFileShareNode extends AzureTreeNodeBase {
    constructor(
    ...
    async getChildren(): Promise<any> {
        ...
        .concat( entries.directories.map((directory: azureStorage.FileService.DirectoryResult) => {
            return new AzureDirectoryNode(directory, this.share, this.storageAccount, this.key, this.getTreeDataProvider(), this);
        }))
        .concat(entries.files.map((file: azureStorage.FileService.FileResult) => {
            return new AzureFileNode(file, this.share, this.storageAccount, this.key, this.getTreeDataProvider(), this);
        }));
    }
