...
import { AzureMessageNode } from './AzureMessageNode';
import * as azureStorage from "azure-storage";
import * as path from 'path';

export class AzureQueueNode extends AzureTreeNodeBase {
    constructor(
    ...
    async getChildren(): Promise<any> {
        ...
        return messages.map((message: azureStorage.QueueService.QueueMessageResult) => {
            return new AzureMessageNode(message, this.queue, this.storageAccount, this.key, this.getTreeDataProvider(), this);
        });
    }
