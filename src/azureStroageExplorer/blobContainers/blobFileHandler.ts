...
export class BlobFileHandler implements IRemoteFileHandler<IAzureNode<BlobNode>> {
    ...
    async uploadFile(node: IAzureNode<BlobNode>, filePath: string) {
        var blobService = azureStorage.createBlobService(node.treeItem.storageAccount.name, node.treeItem.key.value);
        var createOptions: azureStorage.BlobService.createBlockTextBlobRequestOptions = {};
        ...
        await new Promise<string>((resolve, reject) => {
            blobService.createBlockTextBlobFromLocalFile(node.treeItem.container.name, node.treeItem.blob.name, filePath, createOptions, (error: Error, _result: azureStorage.BlobService.BlobResult, _response: azureStorage.ServiceResponse) => {
                if (!!error) {
